...
window.onload = function () {
    ...
    document.getElementById('files').addEventListener('change', function (event) {
        handleFileSelect(event, function(err, data){
            document.getElementById('container_row').style.display = 'block';
            findQ = findQ.bind(null, data);
        });
    }, false);
...
function dropWrap(data, pos, name, number) {
    return htmlData = '<tr data-toggle="collapse" href="#collapse' + number + '"><td>' + pos + '</td>' + '<td>' + data.substring(pos - 10, pos + 50) + '</td>' +
